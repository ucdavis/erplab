...
if ~isempty(EEG.epoch)
      msgboxText{1} =  'pop_creabasiceventlist() has been tested for continuous data only.';
      msgboxText{2} =  '';
      msgboxText{3} =  'HINT: You could use "Export EVENTLIST to text file", instead.';
      ...
      if isfield(EEG.EVENTLIST, 'eventinfo')
            if ~isempty(EEG.EVENTLIST.eventinfo)
                  ...
                  if nargin==1
                        ...
                        question{2} = 'So, pop_creabasiceventlist()  will totally overwrite it?';
...
if nargin==1
      ...
      if isempty(EEG.data)
            msgboxText{1} =  'pop_creabasiceventlist() error: cannot work with an empty dataset!';
