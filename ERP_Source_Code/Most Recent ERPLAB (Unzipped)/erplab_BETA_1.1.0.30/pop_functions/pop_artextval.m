...
if isempty(EEG.data)
      ...
      msgboxText{2} =  'ERROR: pop_artextval() cannot read an empty dataset!';
...
if isempty(EEG.epoch)
      msgboxText{1} =  'Permission denied:';
      msgboxText{2} =  'pop_artextval has been tested for epoched data only';
...
elseif nargin==nvar
      ...
      if size(ampth,1)~=1 || size(ampth,2)~=2
            error('ERROR, you have to specify 2 values for voltage limits. E.g. -100 100');
      end
      if size(flag,1)>1 || size(flag,2)>1
            error('Error: pop_artextval() you have to specify only one flag.')
      end
      if flag==1
            error('Error: pop_artextval() flag 1 is reserved.')
      end
      if flag<0 || flag>16
            error('Error: pop_artextval() flag cannot be greater than 16 or lesser than zero')
      end
else
...
com = sprintf( '%s = pop_artextval( %s, %s, %s, %s, %s);', ...
      inputname(1), inputname(1), testwindow, vect2colon(ampth), chArraystr, num2str(flag));
