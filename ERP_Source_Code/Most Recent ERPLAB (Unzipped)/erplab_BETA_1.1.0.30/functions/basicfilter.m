...
if exist('filtfilt','file') ~= 2
      error('ERPLAB says: error at basicfilter() error. Cannot find the Signal Processing Toolbox');
end
if isempty(EEG.data)
...
if locutoff >= fnyquist
      error('ERPLAB says: Low cutoff frequency cannot be >= srate/2');
end
if hicutoff >= fnyquist
      error('ERPLAB says: High cutoff frequency cannot be >= srate/2');
      
end
if ~typef && filterorder*3 > pnts          % filtfilt restriction
      fprintf('basicfilter: filter order too high');
      error('ERPLAB says: number of samples must be, at least, 3 times the filter order.');
end
