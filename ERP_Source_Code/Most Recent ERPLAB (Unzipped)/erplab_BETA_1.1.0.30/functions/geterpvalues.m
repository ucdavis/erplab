...
% A  = geterpvalues(ERP, latency, binArray, chanArray, op, blc, coi, polpeak, sampeak);
...
if ischar(blc)
        ...
        if isempty(blcnum)
                ...
                if ~ismember(blc,{'no','none','pre','post','all','whole'})
                        msgboxText{1} =  'Invalid baseline range dude!';
...
elseif nargout==2
        ...
        error('ERROR geterpvalues(): Too many output arguments!')
end
...
if length(latsamp)==2
        if latsamp(1)<-1
                msgboxText{1} =  'ERROR: The onset of your latency window is more than 2 samples of difference with the real onset';
        ...
        if latsamp(2)>pnts+2
                msgboxText{1} =  'ERROR: The offset of your latency window is more than 2 samples of difference with the real offset';
                tittle = 'ERPLAB: erp_artmwppth()';
                errorfound(msgboxText, tittle)
                varargout = {[]};
                return
        ...
        if latsamp(1)<-1 || latsamp(1)>2
                msgboxText{1} =  'ERROR: The specified latency is more than 2 samples out of the ERP window.';
                tittle = 'ERPLAB: erp_artmwppth()';
                errorfound(msgboxText, tittle)
                varargout = {[]};
                return
                ...
                for ch = 1:nchan
                        ...
                        if nlat==1
                                ...
                                elseif strcmpi(op,'instabl')
                                        ...
                                        stre1  = 'ERROR in geterpvalues.m: You should enter 2 latencies for ';
                                ...
                                elseif strcmpi(op,'peakampbl')
                                        ...
                                        try
                                                ...
                                                error(sprintf('The requested measurement range (%g - %g +/- %g ms) exceeds the time range of the data (%.1f - %.1f ms).',...
