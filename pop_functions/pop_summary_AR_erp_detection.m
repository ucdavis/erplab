...
if nargin==1
    ...
    if isempty(ERP.erpname)%%changed by GH 2022
        ...
        title    = ['Artifact detection summary for ',ERP.erpname];
    end
    ...
    elseif strcmpi(button,'Save in a file')
        ...
        % Save OUTPUT file
        %
        [filename, filepath, filterindex] = erplab_uiputfile({'*.txt';'*.dat';'*.*'},'Save Artifact Detection Summary as', ['AR_summary_' ERP.erpname]);
