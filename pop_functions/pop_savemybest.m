    ...
    if isempty(fullfilename)
        ...
        try
         ...
         %fullfilename = fullfile(ERP.filepath, ERP.filename);
            ...
            title_msg  = 'ERPLAB: pop_savemymvpa() error:';
            errorfound(msgboxText, title_msg);
            return
        end
    end
...
% Saving
% 
if modegui == 0 % "save-as" filepath has already been gathered from erplab GUI
    if ~isempty(fullfilename)
        %disp(['Saving BESTset at ' fullfilename '...'] )
        [BEST, serror] = saveBEST(BEST, fullfilename, modegui, warnop);
        if serror==1
            return
        end
        issave = 2;
