...
if isempty(observe_ERPDAT.ALLERP)  ||  isempty(observe_ERPDAT.ERP)
    r_ax = axes('Parent', EStudio_gui_erp_totl.ViewAxes,'Color','none','Box','on','FontWeight','normal');
    set(r_ax, 'XTick', [], 'YTick', [],'Box','off', 'Color','none','xcolor','none','ycolor','none');
end
...
if ~isempty(observe_ERPDAT.ALLERP) && ~isempty(observe_ERPDAT.ERP)
    ...
    if isempty(ERPArray)
        ...
        S_erpplot = f_ERPplot_Parameter(observe_ERPDAT.ALLERP,ERPArray);
    ...
    if Bin_chans == 0
        ...
        max_elec_n = observe_ERPDAT.ALLERP(ERPArray(S_ws_getbinchan.Select_index)).nchan;
    else
        elec_n = S_ws_getbinchan.bin_n(S_ws_getbinchan.Select_index);
        max_elec_n = observe_ERPDAT.ALLERP(ERPArray(S_ws_getbinchan.Select_index)).nbin;
    end
...
Current_erp_Index = ERPArray(S_ws_getbinchan.Select_index);
...
if length(ERPArray) ==1
    ...
    if S_ws_getbinchan.Select_index ==1
        ...
        Enable_plus_BackgroundColor = [0 1 0];
        Enable_minus_BackgroundColor = [0 0 0];
    elseif  S_ws_getbinchan.Select_index == length(ERPArray)
        Enable_minus = 'on';
        Enable_plus = 'off';
        Enable_plus_BackgroundColor = [0 0 0];
        Enable_minus_BackgroundColor = [0 1 0];
...
S_ws_getbinchan =  estudioworkingmemory('geterpbinchan');
if ~isempty(CurrentERPindex) &&  numel(CurrentERPindex)==1 && (CurrentERPindex<= numel(ERPArray))
    ...
    if length(ERPArray) ==1
        ...
        elseif  CurrentERPindex == length(ERPArray)
            Enable_minus = 'on';
            Enable_plus = 'off';
            Enable_plus_BackgroundColor = [0 0 0];
            Enable_minus_BackgroundColor = [0 1 0];
