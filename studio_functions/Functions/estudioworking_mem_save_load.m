...
if save_or_load == 1
    ...
    [wm_fname, wm_pathname] = erplab_uiputfile({'*.erpm', 'ERP Studio working memory file (*.erpm)';
...
elseif save_or_load == 2
    ...
    % prompt for path with file browser ui
    [wm_load_fname, wm_load_pathname] = erplab_uigetfile({'*.erpm', 'ERP Studio working memory file (*.erpm)';
    ...
    try
        ...
        wm_loaded = erplab_memory_store('studio', 'normalize', wm_loaded);
        erplab_memory_store('studio', 'save', wm_loaded);
    catch
        wm_loaded = [];
