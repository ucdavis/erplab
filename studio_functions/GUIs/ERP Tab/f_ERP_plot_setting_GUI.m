...
elseif nargin == 1
    ...
    ERP_plotset_box = uiextras.BoxPanel('Parent', varargin{1}, 'Title', 'Plot Settings', 'Padding', 5, 'FontUnits','pixels','FontSize', varargin{2},'BackgroundColor',ColorB_def);
end
...
%%---------------------export layout---------------------------------------
    function gridlayout_export(Source,~)
        ...
        [messgStr,eegpanelIndex] = f_check_erptab_panelchanges();
        if ~isempty(messgStr) && eegpanelIndex==1
            try  observe_ERPDAT.Count_currentERP=eegpanelIndex+1; catch  end%%call the functions from the other panel
        end
...
%%-------------------------------------------------------------------------
    function Count_currentERPChanged(~,~)
        ...
        if ~isempty(ChangeFlag) && ChangeFlag==1
