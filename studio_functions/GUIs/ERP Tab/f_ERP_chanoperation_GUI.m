...
varargout{1} = ERP_chan_operation_gui;
    ...
    function drawui_erp_bin_operation(FontSizeDefault)
        ...
        gui_erp_chan_operation.edit_bineq = uitable(  ...
            ...
            'ColumnName'    , [], ...
            'RowName'       , []);
        set(gui_erp_chan_operation.edit_bineq,'ColumnEditable',true(1,length(dsnames)),'FontSize',FontSizeDefault);
        gui_erp_chan_operation.edit_bineq.KeyPressFcn= @erp_chanop_presskey;
        gui_erp_chan_operation.edit_bineq.CellEditCallback = @equation_table_edited;
        gui_erp_chan_operation.Paras{1} = gui_erp_chan_operation.edit_bineq.Data;

        % Track loaded equation file for history
        gui_erp_chan_operation.loaded_filename = '';  % Empty if equations manually entered
        ...
        gui_erp_chan_operation.ref_asst = uicontrol('Style','pushbutton','Parent',gui_erp_chan_operation.asst_locaInfo,...
            'String','Reference Assist','callback',@ref_asst,'FontSize',FontSizeDefault,'Enable',Enable_label,'BackgroundColor',[1 1 1]); % 2F
        %%%----------------Mode-----------------------------------
        ...
        gui_erp_chan_operation.locaInfor = uicontrol('Style','checkbox','Parent',gui_erp_chan_operation.loca_info_row,...
            'String','Try to preserve location information','callback',@loca_infor,'FontSize',FontSizeDefault,'Value',1,'Enable',Enable_label,'BackgroundColor',ColorB_def); % 2F
        uiextras.Empty('Parent', gui_erp_chan_operation.loca_info_row,'BackgroundColor',ColorB_def);
        ...
        gui_erp_chan_operation.cancel = uicontrol('Style','pushbutton','Parent',gui_erp_chan_operation.run_title,...
            'String','Cancel','callback',@chanop_cancel,'FontSize',FontSizeDefault,'Enable','off','BackgroundColor',[1 1 1]);
        uiextras.Empty('Parent',  gui_erp_chan_operation.run_title,'BackgroundColor',ColorB_def);
...
%%-------------------Equation editor---------------------------------------
    function eq_advanced(Source,~)
        ...
        Eq_Data_cur = gui_erp_chan_operation.edit_bineq.Data;
        Formula_str_cur = {};
        count_cur = 0;
        ...
        def_prev = estudioworkingmemory('pop_erpchanoperator');
        if isempty(def_prev)
            def_prev = { [], 1};
        end
        ...
        if ischar(formulas)
            ...
            gui_erp_chan_operation.loaded_filename = formulas;  % Remember the filename
            gui_erp_chan_operation.equations_modified = false;  % Not modified yet
            ...
            gui_erp_chan_operation.loaded_filename = '';
            gui_erp_chan_operation.equations_modified = false;
        end
...
%%-------------------Equation Load---------------------------------------
    function eq_load(Source,~)
        ...
        gui_erp_chan_operation.loaded_filename = fullname;
        gui_erp_chan_operation.equations_modified = false;
...
%%-------------------Equation Clear---------------------------------------
    function eq_clear(Source,~)
        ...
        gui_erp_chan_operation.loaded_filename = '';
        gui_erp_chan_operation.equations_modified = false;
        estudioworkingmemory('pop_erpchanoperator', {[], 1});
...
%%---------------------Run-------------------------------------------------
    function apply_run(Source,~)
        ...
        % Determine what to pass to pop_erpchanoperator: filename or equations
        % If user loaded from file and hasn't modified, pass filename for cleaner history
        if ~isempty(gui_erp_chan_operation.loaded_filename) && ~gui_erp_chan_operation.equations_modified
            % Pass filename - backend will put 'Filename', 'file.txt' in history
            Formulas_to_pass = gui_erp_chan_operation.loaded_filename;
        else
        ...
        for Numoferp = 1:numel(ERPArray)%%Bin Operations for each selected ERPset
            ...
            [ERP, ERPCOM] = pop_erpchanoperator(ERP, Formulas_to_pass, 'Warning', 'off', 'Saveas', 'off','ErrorMsg', 'command','KeepLocations',keeplocs, 'History', 'gui');
            if isempty(ERPCOM)
                observe_ERPDAT.Process_messg =2;
                return;
            end
...
%%--------------Detect equation table edits--------------------------------
    function equation_table_edited(~,~)
        ...
        gui_erp_chan_operation.equations_modified = true;
    end
