...
drawui_ic_chan_ERP(FontSizeDefault)
varargout{1} = EStudio_erp_box_edit_chan;

    function drawui_ic_chan_ERP(FontSizeDefault)
        try
            [version reldate,ColorB_def,ColorF_def,errorColorF_def] = geterplabstudiodef;
        catch
        ...
        ERP_tab_edit_chan.mode_modify = uicontrol('Style','radiobutton','Parent',ERP_tab_edit_chan.mode_1 ,...
            'String','Modify existing ERPset','callback',@mode_modify,'Value',1,'FontSize',FontSizeDefault,'Enable',Enable_label,'BackgroundColor',ColorB_def); % 2F
        uiextras.Empty('Parent', ERP_tab_edit_chan.mode_1,'BackgroundColor',ColorB_def);
        ...
        ERP_tab_edit_chan.mode_create = uicontrol('Style','radiobutton','Parent',ERP_tab_edit_chan.mode_2 ,...
            'String','Create new ERPset','callback',@mode_create,'Value',0,'FontSize',FontSizeDefault,'Enable',Enable_label,'BackgroundColor',ColorB_def); % 2F
        uiextras.Empty('Parent', ERP_tab_edit_chan.mode_2,'BackgroundColor',ColorB_def);
...
%%---------------------Modify Existing dataset-----------------------------
    function mode_modify(Source,~)
        ...
        if ~isempty(messgStr) && ERPpanelIndex~=15
            observe_ERPDAT.Count_currentERP=ERPpanelIndex+1;%%call the functions from the other panel
        end
