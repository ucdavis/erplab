function varargout = f_ERP_save_as_GUI(varargin)
% F_ERP_SAVE_AS_GUI MATLAB code for f_ERP_save_as_GUI.fig
%      F_ERP_SAVE_AS_GUI, by itself, creates a new F_ERP_SAVE_AS_GUI or raises the existing
%      singleton*.
%
%      H = F_ERP_SAVE_AS_GUI returns the handle to a new F_ERP_SAVE_AS_GUI or the handle to
%      the existing singleton*.
%
%      F_ERP_SAVE_AS_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in F_ERP_SAVE_AS_GUI.M with the given input arguments.
%
%      F_ERP_SAVE_AS_GUI('Property','Value',...) creates a new F_ERP_SAVE_AS_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before f_ERP_save_as_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to f_ERP_save_as_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help f_ERP_save_as_GUI

% Last Modified by GUIDE v2.5 17-Mar-2024 15:42:33

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @f_ERP_save_as_GUI_OpeningFcn, ...
    'gui_OutputFcn',  @f_ERP_save_as_GUI_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before f_ERP_save_as_GUI is made visible.
function f_ERP_save_as_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for f_ERP_save_as_GUI
...
function varargout = f_ERP_save_as_GUI_OutputFcn(hObject, eventdata, handles)
...
if size(DataString,1) < numel(EEGArray)
    ...
    title = 'EStudio: f_ERP_save_as_GUI empty erpname';
    errorfound(msgboxText, title);
    return
end
...
for Numofselected = 1:numel(EEGArray)
    if  isempty(DataString{Numofselected,1})
        ...
        title = 'EStudio: f_ERP_save_as_GUI empty eegname';
        errorfound(msgboxText, title);
        return
    end
end
