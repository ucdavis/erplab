...
elseif nargin == 1
    ...
    box_eegset_gui = uiextras.BoxPanel('Parent', varargin{1}, 'Title', 'EEGsets', 'Padding', 5, 'FontUnits','pixels','FontSize', varargin{2},'BackgroundColor',ColorB_def);
end
...
%%----------------------Refresh alleeg and eeg-----------------------------
    function refresh_eegset(Source,~)
        ...
        EStduio_eegtab_EEG_set.clearall.Enable=Edit_label;
...
%%---------------------Load EEG--------------------------------------------
    function load(~,~)
        ...
        [filename, filepath] = erplab_uigetfile({'*.set','EEG (*.set)'}, ...
            'Load EEG', ...
            'MultiSelect', 'on');
...
%%----------------------Clear ALL selected EEGsets-------------------------
    function clearall(source,~)
        ...
        estudioworkingmemory('f_EEG_proces_messg','EEGsets>Clear');
        observe_EEGDAT.eeg_panel_message =1;
        EEGArray = 1:length(observe_EEGDAT.ALLEEG);
