function varargout = f_EEG_save_single_file(varargin)
% F_EEG_SAVE_SINGLE_FILE MATLAB code for f_EEG_save_single_file.fig
%      F_EEG_SAVE_SINGLE_FILE, by itself, creates a new F_EEG_SAVE_SINGLE_FILE or raises the existing
%      singleton*.
%
%      H = F_EEG_SAVE_SINGLE_FILE returns the handle to a new F_EEG_SAVE_SINGLE_FILE or the handle to
%      the existing singleton*.
%
%      F_EEG_SAVE_SINGLE_FILE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in F_EEG_SAVE_SINGLE_FILE.M with the given input arguments.
%
%      F_EEG_SAVE_SINGLE_FILE('Property','Value',...) creates a new F_EEG_SAVE_SINGLE_FILE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before f_EEG_save_single_file_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to f_EEG_save_single_file_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help f_EEG_save_single_file

% Last Modified by GUIDE v2.5 06-Sep-2023 13:17:11

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @f_EEG_save_single_file_OpeningFcn, ...
    'gui_OutputFcn',  @f_EEG_save_single_file_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before f_EEG_save_single_file is made visible.
function f_EEG_save_single_file_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for f_EEG_save_single_file
...
function varargout = f_EEG_save_single_file_OutputFcn(hObject, eventdata, handles)
...
[fname, pathname] = erplab_uiputfile({'*.set', 'EEGset (*.set)';...
    '*.mat', 'MAT-files (*.mat)';...
...
if isempty(erpname)
    ...
    title = 'EStudio: f_EEG_save_single_file empty erpname';
    errorfound(msgboxText, title);
    return
end
...
if ~isempty(fname) && get(handles.radiobutton_saveas, 'Value')
    ...
    if ~strcmp(ext,'.set') && ~strcmp(ext,'.mat')
        ext = '.set';
    end
    if strcmp(pathstr,'')
        pathstr = cd;
...
elseif isempty(fname) && get(handles.radiobutton_saveas, 'Value')
    ...
    title = 'EStudio: f_EEG_save_single_file empty filename';
    errorfound(msgboxText, title);
    return;
else
    fullname = [];
