...
elseif nargin == 1
    ...
    Eegtab_box_art_sumop = uiextras.BoxPanel('Parent', varargin{1}, 'Title', 'EEG Utilities',...
        'Padding', 5, 'FontUnits','pixels','FontSize', varargin{2},'BackgroundColor',ColorB_def);%, 'HelpFcn', @sumart_help
end
...
%%------------Convert to Continuous EEG----------------------
    function epoch2continuous(Source,~)
        ...
        estudioworkingmemory('f_EEG_proces_messg','EEG Utilities >  Convert to Continuous EEG');
        ...
        estudioworkingmemory('Change2epocheeg',2);
...
%%-----------------Erase undesired event codes---------------------
    function rm_eventcodes(Source,~)
        if  isempty(observe_EEGDAT.EEG) || observe_EEGDAT.EEG.trials >1
            Source.Enable = 'off';
            estudioworkingmemory('f_EEG_proces_messg','EEG Utilities >  Erase undesired event codes: it has been tested for continuous data only');
            observe_EEGDAT.eeg_panel_message =2;
            return;
        end
        [messgStr,eegpanelIndex] = f_check_eegtab_panelchanges();
        ...
        estudioworkingmemory('f_EEG_proces_messg','EEG Utilities >  Erase undesired event codes');
        observe_EEGDAT.eeg_panel_message =1; %%Marking for the procedure has been started.
        ...
        expression = answer{1};
        estudioworkingmemory('pop_eraseventcodes', {expression});
...
%%-------------------Recover bin descriptor file from EEG---------------------
    function rc_bdf(Source,~)
        if  isempty(observe_EEGDAT.EEG) || ~isfield(observe_EEGDAT.EEG,'EVENTLIST') || isempty(observe_EEGDAT.EEG.EVENTLIST)
            Source.Enable = 'off';
            msgboxText = ['EEG Utilities > Recover bin descriptor file from EEG: Your EVENTLIST is empty or does not exist'];
            titlNamerro = 'Warning for EEG Tab';
        ...
        estudioworkingmemory('f_EEG_proces_messg','EEG Utilities >  Recover bin descriptor file from EEG');
        observe_EEGDAT.eeg_panel_message =1; %%Marking for the procedure has been started.
...
%%-------------------Reset Event Code Bytes-------------------
    function event_byte(Source,~)
        if  isempty(observe_EEGDAT.EEG) || observe_EEGDAT.EEG.trials >1
            Source.Enable = 'off';
            erroMessage= 'EEG Utilities >  Reset Event Code Bytes: only works for continuous dataset';
            titlNamerro = 'Warning for EEG Tab';
            estudio_warning(erroMessage,titlNamerro);
            observe_EEGDAT.eeg_panel_message =2;
            return;
        ...
        estudioworkingmemory('f_EEG_proces_messg','EEG Utilities >  Reset Event Code Bytes');
        observe_EEGDAT.eeg_panel_message =1; %%Marking for the procedure has been started.
        EEGArray =  estudioworkingmemory('EEGArray');
        if isempty(EEGArray) ||  any(EEGArray(:) > length(observe_EEGDAT.ALLEEG)) ||  any(EEGArray(:) <1)
            EEGArray = observe_EEGDAT.CURRENTSET;estudioworkingmemory('EEGArray',EEGArray);
        ...
        for Numofeeg = 1:numel(EEGArray)
            ...
            if ischar(EEG.event(1).type)
                erroMessage= ['EEG Utilities >  Reset Event Code Bytes: Your event codes are not numeric for',32,EEG.setname];
                titlNamerro = 'Warning for EEG Tab';
                estudio_warning(erroMessage,titlNamerro);
                observe_EEGDAT.eeg_panel_message =2;
                fprintf( [repmat('-',1,100) '\n']);
...
%%------------------Delete Spurious Additional Responses-------------------
    function rmerrors(Source,~)
        ...
        estudioworkingmemory('f_EEG_proces_messg','EEG Utilities > Delete Spurious Additional Responses');
        observe_EEGDAT.eeg_panel_message =1; %%Marking for the procedure has been started.
