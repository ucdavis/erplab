...
classdef f_editchan_gui < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function load2dataset(app, event)
            ...
            app.UIFigure.Visible = 'off';
            if isfield(EEG,'bindata')
                [lfile,lpath] = erplab_uigetfile({'*.erp','ERP (*.erp)'},'Choose a dataset file that has channel location information',...
                    'MultiSelect', 'off');
            else
                [lfile,lpath] = erplab_uigetfile({'*.set','EEG (*.set)';'*.erp','ERP (*.erp)'},'Choose a dataset file that has channel location information',...
                    'MultiSelect', 'off');
            end
            app.UIFigure.Visible = 'on';
            figure(app.UIFigure);
