...
handles.ERPtooltype=ERPtooltype;
if strcmpi(ERPtooltype,'estudio')
    set(handles.gui_chassis,'Name', ['ABOUT  Estudio ' version ' (' howold ' days old)']);
    handles.pushbutton_erpinfo.String = 'ESTUDIOINFO';
else
    set(handles.gui_chassis,'Name', ['ABOUT  ERPLAB ' version ' (' howold ' days old)']);
    handles.pushbutton_erpinfo.String = 'ERPINFO';
end

p = which('eegplugin_erplab');
p = p(1:findstr(p,'eegplugin_erplab.m')-1);
if strcmpi(ERPtooltype,'estudio')
    fid_about = fopen( fullfile(p, 'GUIs', 'aboutext_estudio.txt'));
else
...
pause(0.2)
if strcmpi(handles.ERPtooltype,'estudio')
