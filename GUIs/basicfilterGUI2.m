...
try handles.nchan=ERPLAB.nbchan;catch, handles.nchan=ERPLAB.nchan;end
...
if length(chx)>length(unique(chx))
      if showmsg            
            ...
            title = 'ERPLAB: basicfilterGUI() error:';
            errorfound(sprintf(msgboxText), title, [1 1 0], 0)
      end
      tf = 0; %
      return
end
...
drawnow; pause(.1)
[outstr v] = read_GUI(hObject, eventdata, handles);
...
%--------------------------------------------------------------------------
function [outstr v] = read_GUI(hObject, eventdata, handles)
...
if ~isempty(highpasscutoff) && ~isempty(lowpasscutoff) && ~isempty(order) && ~isempty(channelArray)
      ...
      msgboxText{1} =  'Something went wrong...Please try again';
      title = 'ERPLAB: basicfilterGUI() error:';
      errorfound(msgboxText, title);
      return
end
...
set(handles.figure1,'Name', ['ERPLAB ' version '   -   Basic Filter GUI for ' typedata])
