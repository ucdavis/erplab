...
handles.listbin     = [];
handles.indxlistbin = []; % channel array
if nbin~=0
    if  isfield(ERPLAB,'bindescr')
        for Numofbin = 1:nbin
            if ~isempty(ERPLAB.bindescr)
                listbin{Numofbin}   = ERPLAB.bindescr{Numofbin};
            else
                listbin{Numofbin}   = ['Bin',32,num2str(Numofbin)];
            end
        end
        handles.listbin     = listbin;
        handles.indxlistbin =def{9};
    end
end
...
if iserpstruct(handles.ERPLAB) && handles.nbin>0
    if isempty(binArray) || any(binArray(:)>handles.ERPLAB.nbin) || any(binArray(:)<1)
        binArray = [1:handles.ERPLAB.nbin];
    end
end
filterallch=0;
...
readobjects = {channelArray, filterallch, highpasscutoff, lowpasscutoff, order, typefilter, remove_dc, boundarycheck, boundarystr,binArray};
...
boundary    = def{8};
colorband   = [0.9922    0.9176    0.7961];
try binArray =  def{9}; catch binArray = []; end
if iserpstruct(ERPLAB) % for erpset
    typedata = 'ERP';
...
if strcmpi(ERPtooltype,'estudio')
    set(handles.gui_chassis,'Name', ['Estudio ' version '   -   Basic Filter GUI for ' typedata]);
else
...
set(handles.radiobutton_fdatafr,'Value', 0);
set(handles.radiobutton_ideal, 'Enable','on')
nchan = handles.nchan;%%et for chans
...
if handles.nbin~=0 && iserpstruct(ERPLAB)
    handles.edit_bin.Enable = 'on';
    handles.pushbutton_bin_browse.Enable = 'on';
    if isempty(binArray) || any(binArray(:)>ERPLAB.nbin) || any(binArray(:)<1)
        binArray=1:ERPLAB.nbin;
    end
    binArraystr   = vect2colon(binArray, 'Delimiter','off', 'Repeat', 'off');
    handles.edit_bin.String = binArraystr;
else
    handles.edit_bin.String = '';
    handles.edit_bin.Enable = 'off';
    handles.pushbutton_bin_browse.Enable = 'off';
end
...
tooltip1  = ['Cutoff frequency = frequency where the magnitude' char(10) 'response of the filter is either 0.5 (-6dB) or 0.707 (-3dB)'];
tooltip2  = ['The filter does not attenuate all frequencies outside the desired frequency range completely;' char(10) ...
    'in particular, there is a region just outside the intended passband where frequencies are attenuated,' char(10) ...
    'but not rejected. This is known as the filter roll-off, and it is usually expressed in dB of attenuation' char(10) ...
    'per octave or decade of frequency. In general, the roll-off for an order-n filter is 6n dB per octave or' char(10) ...
    '20n dB per decade.'];
...
nbin = handles.nbin;
chx   = str2num(handles.edit_bin.String);
if isempty(chx) || any(chx(:)>nbin) || any(chx(:)<1)
    chx = 1:nbin;
end
handles.edit_bin.String = vect2colon(chx, 'Delimiter','off', 'Repeat', 'off');
...
listbin    = handles.listbin;
indxlistbin = handles.indxlistbin;
indxlistbin = indxlistbin(indxlistbin<=length(listbin));
titlename  = 'Select Bin(s)';

if ~isempty(listbin)
    bins = browsechanbinGUI(listbin, indxlistbin, titlename);
    if ~isempty(bins)
        set(handles.edit_bin, 'String', vect2colon(bins, 'Delimiter', 'off'));
        handles.indxlistbin = bins;
        guidata(hObject, handles);
    else
        return
    end
