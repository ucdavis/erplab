...
set(handles.figure1,'Name', ['ERPLAB ' version '   -   ERP Plotting GUI'],...
...
if get(hObject,'Value')
    ...
    if isempty(ERP.binerror)
        msgboxText =  'Permission Denied: %s has no measure of dispersion.\n';
        title = 'ERPLAB: pop_ploterps() missing info:';
        errorfound(sprintf(msgboxText, ERP.erpname), title);
        set(handles.checkbox_stdev,'Value', 0)
        set(handles.edit_std_factor,'Enable','off')
        return
    else
        set(handles.checkbox_stdev,'Value', 1)
        set(handles.edit_std_factor,'Enable','on')        
        valf = get(handles.edit_std_factor,'Value');
        if valf==1
            set(handles.edit_std_factor,'Value',2)
        end
    end
else
    set(handles.edit_std_factor,'Enable', 'off')
...
if ~isempty(binArray) && ~isempty(chanArray)
        ...
        if timew(1)<ERP.xmin*1000 && abs((timew(1)/1000-ERP.xmin)*ERP.srate)>2
                msgboxText = ['Wrong lower time value to plot!\n'...
        ...
        if timew(2)>ERP.xmax*1000 && abs((timew(2)/1000-ERP.xmax)*ERP.srate)>2
                msgboxText =[ 'Wrong upper time value to plot!\n'...
                              'Values will be adjusted.'];
                title = 'ERPLAB: ploterpGUI inputs';
                errorfound(sprintf(msgboxText), title);
        ...
        if errorstd==1
            valf = get(handles.edit_std_factor,'Value');
            if valf>1
                errorstd = get(handles.edit_std_factor, 'Value')-1;
            end
        end       
...
set(handles.edit_std_factor,'String',cellstr(num2str([0:10]'))')
    
if isbinerror && ~istopo
    set(handles.checkbox_stdev,'Enable','on')
    set(handles.edit_std_factor,'Enable','on')
    if errorstd>=1
        set(handles.checkbox_stdev,'Value', 1)
        set(handles.edit_std_factor,'Value',errorstd+1)
    else
        set(handles.checkbox_stdev,'Value', 0)
        set(handles.edit_std_factor,'Value', 1)
    end
else
    set(handles.checkbox_stdev,'Value', 0)
...
%--------------------------------------------------------------------------
function edit_std_factor_Callback(hObject, eventdata, handles)
...
%--------------------------------------------------------------------------
function edit_std_factor_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end
