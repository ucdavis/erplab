...
tooltip1  = ['Each epoch (selected for getting the ERP waveform) is transformed' char(10) 'via fast-Fourier transform (FFT) to power spectrum, and then the ' char(10) ...
    'average across all spectra is derived.'];
tooltip2  = ['The ERP waveform is transformed via fast-Fourier transform (FFT) ' char(10) 'to power spectrum.'];

tooltip3  = ['Windowing functions act on raw data to reduce the effects of the' char(10) 'leakage that occurs during an FFT of the data. Leakage amounts to' char(10) ...
    'spectral information from an FFT showing up at the wrong frequencies.'];
tooltip_DQ  = ['Data Quality measures will be scored for the data going in to the ERP' char(10) ...
    'Recommended default time and measure parameters can be used, or custom times and measures specified' char(10) ...
...
% Save OUTPUT file
%
[fname, pathname] = erplab_uiputfile({'*.txt', 'Text file (*.txt)';...
