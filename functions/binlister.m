% PURPOSE: subroutine for pop_binlister
%
...
%  averaged. It also generates a BINLIST structure  which is added  to
%  the EEGLAB structure (optional).
%
% FORMAT:
%
% [EEG EVENTLIST binOfBins isparsenum] = binlister(EEG, bdfilename, eventlist, neweventlist,...
%                                                  forbiddenCodeArray, ignoreCodeArray, reportable)
%
% Inputs:
%
% EEG                - input dataset
% bdfilename         - bin descriptor file
% eventlist          - name of EventList text file to load (optional). Default='none'
% neweventlist       - name of the new EventList text file to save. Default='none'
% forbiddenCodeArray - prohibited eventcodes. For instance -99 for pauses.
% ignoreCodeArray    - event codes to be ignore by binlister
% reportable         - create a report about binlister's work. 1 yes; 0 no
%
% Outputs:
%
% EEG         - output dataset
% EVENTLIST   - EventList output structure (it was previously added to EEGLAB structure by pop_creaeventlist.m)
% binOfBins   - successful bins counting. For histogram (for instance)
% isparsenum  - numeric parsing of the BIN structure
%
%
% See also pop_binlister.m
%
%
...
function [EEG EVENTLIST binOfBins isparsenum] = binlister(EEG, bdfilename, eventlist, neweventlist, forbiddenCodeArray, ignoreCodeArray, reportable)
...
if ismember(neweventlist,{'none', 'no', ''})
      p = which('eegplugin_erplab');
      path_temp    = p(1:findstr(p,'eegplugin_erplab.m')-1);
      neweventlist = sprintf('eventlist_backup_%g', (datenum(datestr(now))*1e10));    %   ['eventlist_backup_' num2str((datenum(datestr(now))*1e10))];
      neweventlist = fullfile(path_temp, 'erplab_Box', neweventlist);
end
...
elseif nargin>1 && nargin~=7
      error('ERPLAB says: binlister() works with 7 inputs \n')
end
...
if reportable
      ...
      %
      [pr, namer] = fileparts(neweventlist);
...
fprintf('\nPlease wait, Binlister is hunting bins...\n')
...
%                                                 end
                                                ...
                                                if t1>t2  % t1 must be always lesser or equal to t2, either for pre or post home (time increases rightward)
                                                      ...
                                                      if targetLogTime >= t1 && targetLogTime <= t2
                                                            ...
                                                            chance = 2;
                                                            [targetLogPointer, isdetectedLES, writeflag, writeindx, offsetLogitem] = timetest2(EVENTLIST, BIN, les, isnegated, ispqdetected, reportable, fid_report, targetLogCode, previous_t2, ...
...
%       if (100*iLogitem/nitem)>=compareper
%             fprintf('%.1f%%...', 100*iLogitem/nitem);
%             compareper = compareper + 10;
%       end
...
function[targetLogPointer, isdetectedLES, writeflag, writeindx, offsetLogitem] = timetest2(EVENTLIST, BIN, les, isnegated, ispqdetected, reportable, fid_report, targetLogCode, previous_t2, ...
      iLogitem, traffic, islastsequencer, writeflag, writeindx, jBin, kles, mSeq, offsetLogitem, negWord, codestocompareString, forbiddenCodeArray, ...
      ignoreCodeArray, targetLogPointer, targetEventCodeArray, chance)
% I) The basic detection said there was no event code that matches,
% but there is a description of time range for a non-negated code.
