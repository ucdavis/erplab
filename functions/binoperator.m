...
function [ERPout conti cancelop]= binoperator(ERPin, ERPout, expression)
...
if isempty(mater)
      ...
      %
      msgboxText{1} =  'Oops! Sorry, you cannot perform the requested operation in this version.';
...
if isempty(mat) && iswavebin== 0 &&  isavebin== 0 && isharmbin== 0 && ismedianbin== 0 && isstdbin== 0 && iszsbin== 0
      ...
      %
      % Variable setting (no bins)  ---> send to workspace
      %
      ...
      if isempty(mater)
            error(['ERPLAB says: error at binoperator(). Formula ' expression ' contains errors.'])
      end
      ...
      if iswavebin==0  &&  isavebin== 0 && isharmbin== 0 && ismedianbin== 0 && isstdbin== 0 && iszsbin== 0
            ...
            for tk=1:ntok
                  ...
                  if tk>1
                        ...
                        if strcmpi(chspec{tk},'')
                              ...
                              %
                              % look for channels defined at workspace
                              %
                              CHANSP{tk} = evalin('base', chspec{tk});
                        end
            ...
            if ~isempty(indexx)
                  ...
                  if isempty(loc)
                        ...
                        msgboxText{1} =  ['Number of channels is different! Please check bin(s): ' num2str(binpos(loc+1))];
                        tittle = 'ERPLAB: binoperator Error';
                        errorfound(msgboxText, tittle);
                        conti = 0; % No more bin processing...
                        return
      ...
      if ~isempty(nonexistingbinpos)
            msgboxText{1} =  ['Error: Bin(s) [' num2str(binpos(nonexistingbinpos+1)) '] does not exist!!!'];
            msgboxText{2} =  'Only use bins from the list on the right';
            tittle = 'ERPLAB: binoperator error:';
            errorfound(msgboxText, tittle);
            conti = 0; % No more bin processing...
            %    confirma = 0;  % Everything is ok!
            cancelop = 1;
            return
      end
      ...
      for c=1:nchansp
            ...
            if length(labeltemp)>1
                  ...
                  for m=1:length(labeltemp)
                        deflabel = [deflabel '/' labeltemp{1,m}];
                  end
      ...
      if tf(1) && newbin>=1
            ...
            if ~eraser
                  ...
                  if wbmsgon==0
                        ...
                        question{1} = ['Bin ' num2str(newbin) ' already exist!'];
                        question{2} = 'Would you like to overwrite?';
                        tittle      = 'ERPLAB: binoperator, Overwriting Confirmation';
                        button      = askquest(question, tittle, 'Yes');
                  end
                  
            else
                  question{1} = ['Bin ' num2str(newbin) ' will be erased!'];
      ...
      elseif (~tf(1) && newbin>=1 && newbin<=lastslot+1)
            ...
            msgboxText{1} =  ['Error: Bin ' num2str(newbin) ' is out of order!'];
            msgboxText{2} =  '';
            msgboxText{3} =  '"bin" equations must be define in ascending order.';
            msgboxText{4} =  '"nbin" equations must be define in ascending order, from 1 to the highest bin.';
            tittle = 'ERPLAB: binoperator:';
            errorfound(msgboxText, tittle);
            conti = 0; % No more bin processing...
            cancelop = 1;
            return
