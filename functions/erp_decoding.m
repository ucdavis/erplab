...
function [MVPC, ALLMVPC] = erp_decoding(ALLBEST, nIter, nCrossBlocks, DataTimes,relevantChans,classcoding,equalT,ParWorkers,method,BetaWeights,Tooltype,OutcomeMetric,Regularization,normalization_set,TGMPars)
...
for s = 1:nSubs %decoding is performed within each subject independently
    ...
    if method==1
        ...
        fprintf('*** Currently Decoding (LDA) Subject:\t%s ***\n ',sn);
    end
    ...
    % progress checker
    if  strcmpi(equalT,'EqtrlMaxagclasses') || strcmpi(equalT,'EqtrlMaxagbest') || strcmpi(equalT,'EqtrlMaxagfloor')%%GH August 2025 for UnEqualTrialsMaxAVGs
        for Numofbin = 1:BEST.nbin
            blockNall(1,Numofbin) = floor(BEST.n_trials_per_bin(Numofbin)/floor (min(BEST.n_trials_per_bin(:))/nBlocks));
            nPerBin(1,Numofbin)  = BEST.n_trials_per_bin(Numofbin);
        end
    end
    ...
    for iter = 1:nIter
        ...
        if ~strcmpi(Tooltype,'erplab')
            messg = ['Subject:',32, sn,', Iteration:',32,num2str(iter),'/', num2str(nIter)];
        ...
        if   strcmpi(equalT,'EqtrlMaxagclasses') || strcmpi(equalT,'EqtrlMaxagbest') || strcmpi(equalT,'EqtrlMaxagfloor')%%GH August 2025 for UnEqualTrialsMaxAVGs
            %preallocate & rewrite per each iteration
            blockDat_filtData = nan(sum(blockNall(:)),nElectrodes,nSamps);
            labels = nan(sum(blockNall(:)),1);     % bin labels for averaged & filtered EEG data
            blockNum = nan(sum(blockNall(:)),1);   % block numbers for averaged & filtered EEG data
        end
        ...
        for bin = 1:nBins
            ...
            if   strcmpi(equalT,'EqtrlMaxagclasses') || strcmpi(equalT,'EqtrlMaxagbest') || strcmpi(equalT,'EqtrlMaxagfloor')%%GH August 2025 for EqualTrialsMaxAVGs
                nPerBinBlock = floor(nPerBin./blockNall); %array for nPerBin
                min_nPerBinBlock = min(nPerBinBlock(:));
                nPerBinBlock = min_nPerBinBlock*ones(1,numel(nPerBinBlock));
            end
            ...
            if   strcmpi(equalT,'EqtrlMaxagclasses') || strcmpi(equalT,'EqtrlMaxagbest') || strcmpi(equalT,'EqtrlMaxagfloor')%%GH August 2025 for EqualTrialsMaxAVGs
                min_trial_sparsity = floor(true_ntrials/blockNall(bin)); % max
                shuffBin = randperm(min_trial_sparsity*blockNall(bin))';
                x = repmat((1:blockNall(bin))',nPerBinBlock(bin),1); %blockindex
            end
            ...
            mvpc.n_trials_per_class(bin)    = nPerBinBlock(bin);
            ...
            if   strcmpi(equalT,'EqtrlMaxagclasses') || strcmpi(equalT,'EqtrlMaxagbest') || strcmpi(equalT,'EqtrlMaxagfloor')%%GH  August 2025
                nBlocks1111 = blockNall(bin);%%Different bins may have different number of Blocks for this method
            else
                nBlocks1111 = nBlocks;
                blockNall = nBlocks*ones(1,nBins);
            end
            for bl = 1:nBlocks1111
                ...
                blockNum(bCnt) = bl;
                bCnt = bCnt+1;
                data_all_singletrials{bin,bl} = eeg_now(:,:,(unMasktrial));
            end
            mvpc.AvgPerClass(bin) = nBlocks1111;%%GH Sep 2025
        end


        %% Step 7: Loop through each timepoint
        % Do SVM_ECOC at each time point
        parfor (t = 1:nSamps,ParWorkers) %#ok<PFUNK>
            % for t = 1:nSamps
            mdl = [];
            ...
            for i=1:nBlocks % loop through blocks, holding each out as the test set
                ...
                trian_Index = setdiff([1:nBlocks],i);
                ...
                if normalization_set ==1
                    [trnD_nor,tstD_nor] = f_create_train_test_normalization (data_all_singletrials,blockNall,nElectrodes,nBins,i,t);
                    trnD = trnD_nor;
                    tstD = tstD_nor;
                end
                %% Step 3: Training
                if method==1%% SVM%%GH August 2025
                    % template = templateSVM('BoxConstraint', Regularization,'Method','SVM');
                    if classcoding == 2 %onevsall
                        if nBins == 2
                            mdl = fitcsvm(trnD,trnl,'BoxConstraint',Regularization);   %binary decoder
                        else
                            template = templateSVM('BoxConstraint', Regularization);
                            mdl = fitcecoc(trnD,trnl,'Learners',template, 'Coding','onevsall'); %train support vector machine
                        end

                    elseif classcoding == 1 %onevsone
                        template = templateSVM('BoxConstraint', Regularization);
                        mdl = fitcecoc(trnD,trnl, 'Coding','onevsone','Learners',template );   %train support vector machine
                    else
                        error('Decoding Toolbox has unspecified SVMcoding');
                    end
                else%%LDA algorithm GH Sep. 2025
                    mdl = fitcdiscr(trnD,trnl,'Gamma',Regularization);
                end

                %% Step 5: Testing
                [LabelPredicted,LabelScores] = predict(mdl, tstD);    % predict classes for new data
                svm_scores(iter,i,t,:,:) = LabelScores; %%save scores for each fold and each iteraction; %% GH 2025 August
                svm_predict(iter,i,t,:) = LabelPredicted;  % save predicted labels

                tst_target(iter,i,t,:) = tstl;             % save true target labels

                % % Now test on all time points for temporal generalization matrix  %%GH Dec. 2025
                if TGMPars==1
                    [svm_predict_TGM1,tst_target_TGM1,svm_scores_TGM1] = f_test_all_times_TGM(blockDat_filtData,blockNum,i,mdl,tstl);
                    svm_predict_TGM(iter,i,t,:,:) = svm_predict_TGM1;
                    tst_target_TGM(iter,i,t,:,:)  = tst_target_TGM1;
                    svm_scores_TGM(iter,i,t,:,:,:) = svm_scores_TGM1;
                end
                if saveweights == 1
                    BetaWeights_Raw(iter,t,i,:) = f_decoding_betaweights(mdl,nDecoders,method,nBins);%%uncorrected weights GH Dec 2025
                end
    ...
    if saveweights == 1%%GH Jan. 2026
        BetaWeights_Raw = squeeze(mean(mean(BetaWeights_Raw,1),3));
        mvpc.details.BetaWeightsRaw =  BetaWeights_Raw;
    else
        mvpc.details.BetaWeightsRaw = [];
    end
    ...
    if strcmpi(OutcomeMetric,'ACC')%%GH 2025 August
        ...
        mvpc.DecodingUnit= 'AUC';%%GH 2025 August
        % svm_scores = [];
        mvpc.chance = 0.5;%%GH 2026 March
    end
    mvpc = avgconfusionSVM(mvpc,tst_target, svm_predict);
    if method==2%%GH 2025 August
        mvpc.DecodingMethod = 'LDA';
    end
    mvpc.details.regularization = Regularization;%%GH Sep 2025
    mvpc.details.normalization  = normalization_set;%%GH Sep 2025
    ...
    %% Step 9: Compute Temporal Generalization Matrices (ACC & AUC) GH Dec. 2025
    % Works for both binary and multi-class decoding
    if TGMPars==1
        TGM_erp =  f_compute_TGM_mvpa(mvpc,svm_predict_TGM,tst_target_TGM,svm_scores_TGM,OutcomeMetric);
        mvpc.TGM  = TGM_erp;
    end
    svm_predict_TGM =[];tst_target_TGM = [];svm_scores_TGM = [];
