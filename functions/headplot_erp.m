%
%
% Modified headplot.m from Arnaud Delorme, Colin Humphries, Scott Makeig, SCCN/INC/UCSD, La Jolla, 1998, to be used by ERPLAB
%
%

function [HeadAxes, ColorbarHandle] = headplot_erp(values, arg1, varargin)
...
%
    ...
    g = finputcheck(varargin(2:end), { 'orilocs'      'string'  { 'on','off' }             'off';
                                       ...
                                       'comment'      'string'  []                         '' }, 'headplot_erp', 'ignore');
    if isstr(g), 
        error(g);
    end;
...
%   
   ...
   g = finputcheck( varargin, { ...
       ...
       'labels'     'integer' [0 1 2]        0 }, 'headplot_erp');
