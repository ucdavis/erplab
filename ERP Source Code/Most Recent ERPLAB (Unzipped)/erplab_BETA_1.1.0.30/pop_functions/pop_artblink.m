...
elseif nargin==nvar
      ...
      if size(flag,1)>1 || size(flag,2)>1
            error('Error: pop_artblink() you have to specify only one flag.')
      end
      if flag==1
            error('Error: pop_artblink() flag 1 is reserved.')
      end
      if flag<0 || flag>16
            error('Error: pop_artblink() flag cannot be greater than 16 or lesser than zero')
      end
else
...
com = sprintf( '%s = pop_artblink( %s, [%s], %s, %s, %s, %s);', ...
      ...
      num2str(ccovth), chArraystr, num2str(flag));
