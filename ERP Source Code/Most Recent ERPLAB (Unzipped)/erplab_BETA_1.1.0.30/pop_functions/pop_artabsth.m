% Usage
%
% >> EEG = pop_artabsth(EEG, twin, ampth, winms, stepms, chan, flag);
%
% Inputs
%
% EEG       - input dataset
...
function [EEG com] = pop_artabsth(EEG, twin, ampth, chan, flag)
...
if isempty(EEG.data)
      ...
      msgboxText{2} =  'ERROR: pop_artabsth() cannot read an empty dataset!';
      title = 'ERPLAB: pop_artabsth';
...
if isempty(EEG.epoch)
      msgboxText{1} =  'Permission denied:';
      msgboxText{2} =  'pop_artabsth has been tested for epoched data only';
      title = 'ERPLAB: pop_artabsth Permission';
      errorfound(msgboxText, title);
      return
end
...
if nargin <nvar
      ...
      defx = {[EEG.xmin*1000 EEG.xmax*1000] 100 100 50  1:EEG.nbchan 0}; % in case of reset
...
elseif nargin==nvar
      ...
      if size(flag,1)>1 || size(flag,2)>1
            error('Error: pop_artabsth() you have to specify only one flag.')
      end
      if flag==1
            error('Error: pop_artabsth() flag 1 is reserved.')
      end
      if flag<0 || flag>16
            error('Error: pop_artabsth() flag cannot be greater than 16 or lesser than zero')
      end
else
      error('Error: pop_artabsth() works with 7 arguments')
end
...
com = sprintf( '%s = pop_artabsth( %s, %s, %s, %s, %s, %s, %s);', ...
