...
elseif nargin==nvar
      ...
      if size(flag,1)>1 || size(flag,2)>1
            error('Error: pop_artderiv() you have to specify only one flag.')
      end
      if flag==1
            error('Error: pop_artderiv() flag 1 is reserved.')
      end
      if flag<0 || flag>16
            error('Error: pop_artderiv() flag cannot be greater than 16 or lesser than zero')
      end
else
...
com = sprintf( '%s = pop_artderiv( %s, %s, %s, %s, %s);', ...
      inputname(1), inputname(1), testwindow, num2str(ratechange), chArraystr, num2str(flag));
