...
if nmastercode~=nnewcode || nnewcode~=length(newlate)
        error('ERPLAB says: insertcodearound() -> mastercode, newcode, and newlate must be of same lenght')
end
        ...
        if ischar(auxevent(1).type) % string eventcodes
                ...
                if ischar(mastercodeK)
                        ...
                        if isempty(mastercodeK)
                                error('ERPLAB says: Error,  Your event codes are numerical, so your master code have to be as well.')
                        else
                ...
                if ischar(newcodeK)
                        ...
                        if isempty(newcodeK)
                                error('ERPLAB says: Error, Your event codes are numerical, so your new code have to be as well.')
                        else
                                fprintf('\n\nWARNING: master code was converted into numeric code to match the EEG.event current format.\n\n')
                        end
                end
