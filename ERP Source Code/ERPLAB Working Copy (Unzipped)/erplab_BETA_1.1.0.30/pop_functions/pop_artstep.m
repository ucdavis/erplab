...
%  Calculates the difference between the average of the first half moving window and the average of
%  the second half (in uV), per each epoch, at the specified channel(s).
%
...
if nargin <nvar
        ...
        if size(flag,1)>1 || size(flag,2)>1
                msgboxText{1} =  'you have to specify only one flag.';
        ...
        if flag==1
                msgboxText{1} =  'flag 1 is reserved.';
                title = 'ERPLAB: Flag input error';
                errorfound(msgboxText, title);
                return
        end
        ...
        if flag<0 || flag>16
                msgboxText{1} =  'flag cannot be greater than 16 nor lesser than 0';
...
elseif nargin==nvar
        ...
        if size(flag,1)>1 || size(flag,2)>1
                error('ERPLAB says: pop_artstep() you have to specify only one flag.')
        end
        if flag==1
                error('ERPLAB says: pop_artstep() flag 1 is reserved.')
        end
        if flag<0 || flag>16
                error('ERPLAB says: pop_artstep() flag cannot be greater than 16 or lesser than zero')
        end
