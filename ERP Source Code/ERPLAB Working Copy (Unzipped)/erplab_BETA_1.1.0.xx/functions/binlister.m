...
function [EEG EVENTLIST binOfBins] = binlister(EEG, bdfilename, eventlist, neweventlist, forbiddenCodeArray, ignoreCodeArray, reportable)

%binlister(EEG, file1, file2, file3, forbiddenCodeArray, ignoreCodeArray, reportable);

version    = geterplabversion;

if strcmp(neweventlist,'')
        p = which('eegplugin_erplab');
        path_temp    = p(1:findstr(p,'eegplugin_erplab.m')-1);
        neweventlist = ['eventlist_backup_' num2str((datenum(datestr(now))*1e10))];
        neweventlist = fullfile(path_temp, 'erplab_Temp', neweventlist);
end
...
elseif nargin>1 && nargin~=7
        fprintf('Error: binlister works with 7 inputs \n')
        return
end
...
if isparsednumerically
        ...
        if reportable
                ...
                %
                [pr, namer, extr, versn] = fileparts(neweventlist);
                                        ...
                                        while (kles <= nLes) && isdetectedLES   % fields: {prehome} -->{posthome}
                                                ...
                                                while (kSeq <= nSequencer) && (isdetectedLES) % each sequencer
                                                        ...
                                                        if EVENTLIST.eventinfo(targetLogPointer).enable~=-1 && ~isignoredc % is enable the current event code to test?
                                                                ...
                                                                condfprintf(reportable, fid_report,'Comparing: %s with %g ...\n',...
                                                                ...
                                                                elseif CC         % The basic detection said there is no match for the specified event code and there is no time range specified.
                                                                        ...
                                                                        if isnegated   % 21 February 2008
                                                                                condfprintf(reportable, fid_report,...
                                                                                        'Hey! good news! event code %s is/are different(s)!\n', codestocompareString);
                                                                                isdetectedLES = 1;
