...
if nformulas==0
        msgboxText{1} =  sprintf('Error: There is not any formula!');
        tittle = 'ERPLAB: checkformulas() error:';
        errorfound(msgboxText, tittle)
        return
end
...
if ~isempty(tokrightnew)
        msgboxText{1} =  sprintf('Error: You cannot use "n%s" sintax on the right side of the formulas.', exp03);
        tittle = sprintf('ERPLAB: %s() error:', funcname);
        errorfound(msgboxText, tittle)
        return
end
...
elseif ntokleft==ntokleftnew && ntokleftnew>0 && editormode==0
        % Create chans/bins at a new EEG/ERP does not match with mode option
        msgboxText{1} =  sprintf('Error: %s cannot modify an existing %s using "n%s" sintax.', funcname, datatype, exp03);
        tittle = sprintf('ERPLAB: %s() error:', funcname);
        errorfound(msgboxText, tittle)
        return
...
elseif ntokleftnew==0 && ntokleft>0 && editormode==1
        % Create chans/bins at a new EEG/ERP  does not match with mode option
        msgboxText{1} =  sprintf('Error: %s cannot create a new %s without "n%s" sintax.', funcname, datatype, exp03);
        tittle = sprintf('ERPLAB: %s() error:', funcname);
        errorfound(msgboxText, tittle)
        return
