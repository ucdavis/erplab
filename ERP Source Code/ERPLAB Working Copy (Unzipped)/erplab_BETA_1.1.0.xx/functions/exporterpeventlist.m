...
%
%   ERP       - input dataset
% evefilename - name of EventList file to save (*.txt)
% evefilepath - path of EventList file to save
%
...
function exporterpeventlist(ERP, indexel, lfname)

fprintf('exporterpeventlist.m : START\n');
...
if nargin~=3
        error('Error exporterpeventlist.m : Incorrect inputs')
end

if ~iserpstruct(ERP)
        error('Error exporterpeventlist.m : Incorrect inputs - ERP structure is requiered')
end

if ~isfield(ERP, 'EVENTLIST')
        error('Error exporterpeventlist.m : Incorrect inputs - EVENTLIST structure is requiered')
end

if strcmp(lfname, '')
        error('Error exporterpeventlist.m : Incorrect inputs - file name is requiered')
end

[evefilepath, evefilename, ext, versn] = fileparts(lfname);
...
if wf ==1
        ...
        fprintf(fid_eventlist,'#                                                 (sec)           (sec)     (msec)    (binary)   (binary)\n');
        fprintf(fid_eventlist,'\n\n');
...
fprintf('exporterpeventlist.m : END\n');
fprintf('Done.\n');
