...
if nargin==1
      ...
      if iseegstruct(EEG)
            ...
            if ~isempty(binhunter) && (option2do==0 || option2do==2)
                  msgboxText{1} =  '      This dataset already has bins assigning. ';
                  msgboxText{2} =  'Would you like to overwrite this bins?';
                  title = 'ERPLAB: Error';
...
if nnz(binofbins)>=1
      ...
      if ~isempty(EVENTLIST)
            ...
            msgboxText{1} =  '      Bin descriptor file contains errors! ';
            msgboxText{2} =  'For details, please read command window messages.';
      ...
      msgboxText{1} =  'Bins were not found! ';
      msgboxText{2} =  'Try with other BDF or modify the current one';
