% Usage
%
% >> EEG = pop_artflatline(EEG, twin, ampth, chan, flag);
%
% Inputs
%
% EEG       - input dataset
...
if nargin <nvar
      ...
      prompt    = {'Test period (start end) [ms]','Peak-to-peak threshold (one value, e.g. 2):', 'Duration [ms]', 'Channel(s)'};
      ...
      if size(flag,1)>1 || size(flag,2)>1
            msgboxText =  'You have to specify only one flag.';
            title = 'ERPLAB: Flag input error';
            errorfound(msgboxText, title);
            return
      end
      if flag==1
            msgboxText =  'Flag 1 is reserved.';
            title = 'ERPLAB: Flag input error';
            errorfound(msgboxText, title);
            return
      end
      if flag<0 || flag>16
            msgboxText =  'Flag cannot be greater than 16 nor lesser than 0';
            title = 'ERPLAB: Flag input error';
            errorfound(msgboxText, title);
            return
      end
...
elseif nargin==nvar
      ...
      if size(flag,1)>1 || size(flag,2)>1
            error('ERPLAB says: error at pop_artflatline(), you have to specify only one flag.')
      end
      if flag==1
            error('ERPLAB says: error at pop_artflatline(), flag 1 is reserved.')
      end
      if flag<0 || flag>16
            error('ERPLAB says: error at pop_artflatline(), flag cannot be greater than 16 or lesser than zero')
      end
else
...
if flag<0 || flag>16
      error('ERPLAB says: error at pop_artflatline(), flag index cannot be greater than 16 or lesser than zero')
end
...
% [EEG com] = pop_artflatline(EEG, twin, ampth, dur, chan, flag)

com = sprintf( '%s = pop_artflatline( %s, %s, %s, %s, %s, %s);', ...
      inputname(1), inputname(1), testwindow, vect2colon(ampth), num2str(dur), chArraystr, num2str(flag));
