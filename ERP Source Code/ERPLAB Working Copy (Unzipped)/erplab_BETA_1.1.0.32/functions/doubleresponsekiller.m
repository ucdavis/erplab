...
if isempty(EEG.data)
        error('doubleresponsekiller error: cannot work with an empty dataset')
end

if ~isempty(EEG.epoch)
        error('doubleresponsekiller only works on continuous data!')
end
...
if ischar(auxevent(1).type)
        ...
        if isnumeric(code)
                error(['doubleresponsekiller error: Your event codes are string, but you specify a numeric code ' num2str(code)])
        end
        ...
        if ischar(code)
                error(['doubleresponsekiller error: Your event codes are numeric, but you specified a string code ' code])
        end
...
elseif strcmpi(pos,'last')
        ...
        error('doubleresponsekiller only recognize "first" and "last" as position option')
end
