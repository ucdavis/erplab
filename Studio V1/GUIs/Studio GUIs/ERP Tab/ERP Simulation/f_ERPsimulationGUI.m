%%This is part of pop_ERP_simulation.
...
function varargout = f_ERPsimulationGUI(varargin)
% F_ERPSIMULATIONGUI MATLAB code for f_ERPsimulationGUI.fig
%      F_ERPSIMULATIONGUI, by itself, creates a new F_ERPSIMULATIONGUI or raises the existing
%      singleton*.
%
%      H = F_ERPSIMULATIONGUI returns the handle to a new F_ERPSIMULATIONGUI or the handle to
%      the existing singleton*.
%
%      F_ERPSIMULATIONGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in F_ERPSIMULATIONGUI.M with the given input arguments.
%
%      F_ERPSIMULATIONGUI('Property','Value',...) creates a new F_ERPSIMULATIONGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before f_ERPsimulationGUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to f_ERPsimulationGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help f_ERPsimulationGUI

% Last Modified by GUIDE v2.5 24-Mar-2023 12:38:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @f_ERPsimulationGUI_OpeningFcn, ...
    'gui_OutputFcn',  @f_ERPsimulationGUI_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before f_ERPsimulationGUI is made visible.
function f_ERPsimulationGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for f_ERPsimulationGUI
...
try
    ...
    %%ampwhite, pinkop,amppink,sinop,sinamp,sinfre
end
if isempty(def)
    def  = {1,1,100,50,1,-200,799,1,1000,0,1,0,1,0,1,10};
end
...
function varargout = f_ERPsimulationGUI_OutputFcn(hObject, eventdata, handles)
varargout{1} = [];
...
handles.output = {BasicFunOp,Amp_bas,Mean_bas,SD_bas,Tau_bas,EpochStart,EpochStop,...
    srateOp,srate,WhiteOp,WhiteAmp,pinkOp,pinkAmp,sinOp,sinAmp,sinFre};
% Update handles structure
guidata(hObject, handles);
...
if handles.radiobutton_siniose.Value
    ...
    X =  Times/1000;
    Desirednosizesin = PeakAmp*sin(2*FreHz*pi*X);
end
...
[ERP, ALLERPout, ERPCOM] = pop_loaderp('filename', filename, 'filepath', filepath, 'Warning', 'on','multiload', 'on',...
    'History', 'off');
if ~isempty(ALLERPout)
    ALLERP =  handles.ALLERP;
    
    if ~isempty(ALLERP)
        ERPNum  = length(ALLERP);
        for Numoferp = 1:length(ALLERPout)
            ALLERP(ERPNum+Numoferp)  = ALLERPout(Numoferp);
        end
    else
        ALLERP = ALLERPout;
    end
    ...
    handles.checkbox_ERP_op.Enable = 'on';
    handles.edit_erpset.Enable = 'on';
    handles.pushbutton_erpset.Enable = 'on';
    handles.edit_channel.Enable = 'on';
    handles.pushbutton_channel.Enable = 'on';
    ...
    ERP = ALLERP(CURRENTERP);
    %%check chan index
    chanArray = str2num(handles.edit_channel.String);
    ...
    if handles.checkbox_ERP_op.Value ==1
        Enableb = 'off';
        handles.edit_epoch_start.String = num2str(ERP.times(1));
        handles.edit_epochstop.String = num2str(ERP.times(end));
        handles.edit_srate.String = num2str(ERP.srate);
        if ERP.srate>0
            handles.edit_speriod.String = num2str(1000/ERP.srate);
        end
    else
        Enableb = 'on';
    end
    %%disable "Basic information for simulation". That is, the parameters
    %%must be the same to the real ERPset
    handles.edit_epoch_start.Enable = Enableb;
    handles.edit_epochstop.Enable = Enableb;
    handles.radiobutton_srate.Enable = Enableb;
    handles.edit_srate.Enable = Enableb;
    handles.radiobutton_speriod.Enable = Enableb;
    handles.edit_speriod.Enable = Enableb;
    guidata(hObject, handles);
    plotsimulationwave(hObject, eventdata, handles);
else
    return;
